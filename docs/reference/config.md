...
- `sql_driver`:
  - Either `github.com/jackc/pgx/v4`, `github.com/jackc/pgx/v5`, `github.com/lib/pq`, `github.com/go-sql-driver/mysql` or `github.com/duckdb/duckdb-go/v2`. No defaults, except for the `duckdb` engine, which generates for `github.com/duckdb/duckdb-go/v2` through `database/sql` and accepts nothing else. Required if query annotation `:copyfrom` is used.
- `emit_db_tags`:
