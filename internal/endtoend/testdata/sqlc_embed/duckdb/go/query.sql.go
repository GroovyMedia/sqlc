...
const returning = `-- name: Returning :one
...
func (q *Queries) Returning(ctx context.Context, arg ReturningParams) (ReturningRow, error) {
	row := q.db.QueryRowContext(ctx, returning, arg.ID, arg.Name, arg.Age)
	var i ReturningRow
	err := row.Scan(&i.User.ID, &i.User.Name, &i.User.Age)
	return i, err
}
...
const withParam = `-- name: WithParam :many
SELECT p.id, p.user_id, u.name FROM posts AS p
INNER JOIN users AS u ON p.user_id = u.id
WHERE u.name = $1;
`
...
func (q *Queries) WithParam(ctx context.Context, name any) ([]WithParamRow, error) {
	...
	defer rows.Close()
	var items []WithParamRow
	for rows.Next() {
		var i WithParamRow
		if err := rows.Scan(&i.Post.ID, &i.Post.UserID, &i.Name); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
